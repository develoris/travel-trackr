...
const buildBackupFolderName = (date = new Date()) => {
...
export const runMongoBackup = async ({
  ...
  dbName = process.env.BACKUP_DB_NAME || getDbNameFromUri(process.env.MONGODB_URI || DEFAULT_URI),
  ...
  try {
    ...
    // Backup completo: una collection = un file JSON indipendente.
    ...
    await writeFile(join(backupDir, "metadata.json"), JSON.stringify(metadata, null, 2), "utf8");

    logger.log(`[backup] Completed: ${results.length} collections in ${backupDir}`);
    return metadata;
  } finally {
    await client.close();
  }
