...
export const postUser = async (_req: Request, _res: Response): Promise<never> => {
  throw new AppError({
...
export const signIn = async (_req: Request, _res: Response): Promise<never> => {
  throw new AppError({
    code: "REGISTRATION_DISABLED",
    status: 403,
...
const setRefreshTokenCookie = (res: Response, refreshToken: string): void => {
  const maxAge =
    Number(process.env.REFRESH_TOKEN_DAYS || 30) * 24 * 60 * 60 * 1000;
...
export const login = async (req: Request, res: Response): Promise<Response> => {
  ...
  return res
    .status(200)
    .json({ message: "Login effettuato", user, accessToken: tokens.accessToken });
};
...
export const patchUser = async (req: Request, res: Response): Promise<Response> => {
  const user = await updateUserById(String(req.params.id), req.body as Record<string, unknown>);
  ...
  if (!user) {
    throw new AppError({
      ...
      status: 404,
      userMessage: "Utente non trovato.",
      developerMessage: `User ${String(req.params.id)} not found for update`
    });
  }
...
export const removeUser = async (req: Request, res: Response): Promise<Response> => {
  const user = await deleteUserById(String(req.params.id));
  ...
  if (!user) {
    throw new AppError({
      ...
      status: 404,
      userMessage: "Utente non trovato.",
      developerMessage: `User ${String(req.params.id)} not found for delete`
    });
  }
